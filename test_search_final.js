...
const mockLLM = {
  async chatWithTools(messages, tools, options) {
    ...
    // 返回search工具调用，直接指定测试文件
    return {
      message: {
        content: "Searching for pattern in test file...",
        toolCalls: [
          {
            id: "test-search-1",
            name: "search",
            arguments: JSON.stringify({
              ...
              path: "/tmp/test_search_viber.txt",
...
async function main() {
  ...
  try {
    const result = await agent.run("Search for 'search pattern' in test file");
    console.log("\n=== Search Test Results ===");
